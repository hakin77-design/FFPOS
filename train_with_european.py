...
# Load European matches for training
print('Loading European matches...')
...
model.eval()
with torch.no_grad():
    outputs = model(X[:1000])
    _, predicted = torch.max(outputs.data, 1)
    accuracy = (predicted == y[:1000]).sum().item() / 1000
    print(f'Final test accuracy (1000 samples): {accuracy*100:.2f}%')
